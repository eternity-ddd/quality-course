#!/usr/bin/env bash
# 백엔드(8080) + 프론트엔드(5173)를 한 번에 띄운다. (macOS / Linux)
set -uo pipefail
cd "$(dirname "$0")"
...
if lsof -ti tcp:8080 >/dev/null 2>&1 || lsof -ti tcp:5173 >/dev/null 2>&1; then
  echo "⚠️  8080/5173 포트가 이미 사용 중입니다. 먼저 ./stop.sh 를 실행하세요."
  exit 1
fi
...
if curl -s --retry 60 --retry-delay 2 --retry-connrefused --retry-all-errors \
     "http://localhost:8080/api/shops?lat=37.4979&lng=127.0276&size=1" -o /dev/null; then
  echo "  ✅ 백엔드 OK (http://localhost:8080)"
else
  echo "  ⚠️ 백엔드 준비 시간 초과 — logs/backend.log 확인"
fi

echo "▶ 프론트 준비 대기..."
if curl -s --retry 30 --retry-delay 1 --retry-connrefused "http://localhost:5173" -o /dev/null; then
  echo "  ✅ 프론트 OK"
else
...
echo
echo "✅ 실행 완료 → 브라우저에서  http://localhost:5173  접속"
echo "   중단하려면  ./stop.sh"
