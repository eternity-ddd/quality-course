...
음식 배달 도메인을 **절차적(procedural) 스타일**로 구현한 예제 프로젝트입니다.
(객체지향/DDD 스타일로 같은 도메인을 구현한 `delivery-oo`와 비교용 한 쌍입니다.)
...
- 가게 "내 주변 검색"은 MySQL 공간함수(`ST_Distance_Sphere`) 대신 위/경도 컬럼 기반 **Haversine 식**으로 계산합니다 (`ShopRepository.findNearbyWithDistance`).
